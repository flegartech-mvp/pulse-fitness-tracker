...
import { parseNonNegativeNumberInput, parsePositiveIntegerInput } from '../utils/validation';
...
export default function WorkoutScreen({ navigation }) {
  ...
  function addSetToDraft() {
    ...
    const weight = parseNonNegativeNumberInput(weightInput);

    if (reps === null || weight === null) {
      setDraftError('Use whole-number reps and a weight from 0 to 1000 kg.');
  ...
  function commitExercise() {
    if (!draftName.trim()) {
      setDraftError('Exercise name is required.');
      Alert.alert('Missing Name', 'Enter the exercise name.');
      return;
    }
    if (draftSets.length === 0) {
      setDraftError('Add at least one set before saving this exercise.');
    ...
    >
      <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={styles.scroll}>
        ...
        <AppButton
          ...
          accessibilityLabel="Repeat last exercise"
        />
        )}
