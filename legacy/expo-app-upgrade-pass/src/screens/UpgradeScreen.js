...
export default function UpgradeScreen() {
  ...
  async function openPayPalAmount(amount) {
    const url = `${PAYPAL_BASE}${amount}`;
    const canOpen = await Linking.canOpenURL(url);
    if (!canOpen) {
      throw new Error('PayPal link is not available on this device.');
    }
  ...
  async function handlePayWithPayPal(plan) {
    ...
    try {
      await openPayPalAmount(plan.paypalAmount);
      setStep('confirm');
    } catch (err) {
      console.error('handlePayWithPayPal:', err);
      Alert.alert('Could not open PayPal', err.message || 'Open PayPal manually and try again.');
    }
  }
    ...
    return (
      <View style={styles.container}>
        <View style={styles.activeCard}>
          <Text style={styles.activeIcon}>PRO</Text>
          ...
          <Text style={styles.activePlan}>
            {type === 'session_pack'
              ...
              : type === 'monthly'
                ? `Monthly - renews ${expiry ?? 'pending'}`
                : `Yearly - renews ${expiry ?? 'pending'}`}
          </Text>
  ...
  if (step === 'confirm' && selectedPlan) {
    return (
      <View style={[styles.container, styles.confirmContainer]}>
        <Text style={styles.confirmIcon}>PAY</Text>
        ...
        <Pressable
          ...
          accessibilityRole="button"
          accessibilityLabel="Back to premium plans"
        >
          <Text style={styles.backLinkText}>Back to plans</Text>
        </Pressable>
        <Pressable
          style={styles.reopenLink}
          onPress={() => openPayPalAmount(selectedPlan.paypalAmount).catch(err => {
            Alert.alert('Could not open PayPal', err.message || 'Open PayPal manually and try again.');
          })}
          accessibilityRole="button"
          accessibilityLabel="Re-open PayPal"
        >
